...
class ListPage:
    ...
    def get_table_rows(self):
        return self.test.browser.find_elements(By.CSS_SELECTOR,
                                               '#id_list_table tr')
    ...
    def get_share_box(self):
        return self.test.browser.find_element(
            By.CSS_SELECTOR, 'input[name="share"]'
        )
    ...
    def share_list_with(self, email):
        self.get_shared_box().send_keys(email)
        self.get_shared_box().send_keys(Keys.ENTER)
        self.test.wait_for(lambda: self.test.assertIn(
            email,
            [item.text for item in self.get_shared_with_lists()]
        ))
